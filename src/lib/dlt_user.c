...
pthread_mutex_t g_mutex_logOnWrite = PTHREAD_MUTEX_INITIALIZER;
...
static int dlt_offline_on_init(const char* path);
static int dlt_offline_on_write(DltOfflineFileList *file, void* data1, int size1, void* data2, int size2);
static void dlt_offline_on_clear();
...
{
    dlt_user.dlt_is_daemon = 1;
}

void dlt_disable_daemon(void)
{
    dlt_user.dlt_is_daemon = 0;
}
...
{
    return dlt_init_file_dir(name, 5,  1073741824); // 1024 *1024 * 1024 = 1073741824
}
...
DltReturnValue dlt_init_file_dir(const char *path, int32_t file_count, int64_t file_size)
{
    ...
    dlt_user.file_count = file_count;
    dlt_user.file_size = file_size;
    /* open DLT output file */
    if (DLT_RETURN_OK != dlt_offline_on_init(path))
    {
        return DLT_RETURN_ERROR;
    }
    
    if (dlt_user.dlt_file_handle == 0)
    {
...
DltReturnValue dlt_init_common(void)
{
    ...
    dlt_user.file_count = 5;
    dlt_user.file_size = 1073741824; // 1024 *1024 * 1024 = 1073741824
    dlt_user.file_list = 0;
    dlt_user.dlt_file_handle=0;
    dlt_user.dlt_log_handle = -1;
...
#endif
        if (dlt_user.dlt_file_handle)
        {
            dlt_offline_on_clear();
            dlt_user.dlt_file_handle = 0;
        }
...
{
    uint32_t type_info = DLT_SCOD_UTF8;
    const VarInfo var_info = { name, unit, true };
    return dlt_user_log_write_generic_attr(log, data, datalength, type_info, &var_info);
}
//new add
...
{
    uint32_t type_info = DLT_SCOD_ASCII;
    const VarInfo var_info = { name, unit, true };
    return dlt_user_log_write_generic_attr(log, data, datalength, type_info, &var_info);
}
...
DltReturnValue dlt_user_log_send_log(DltContextData *log, int mtype)
{
    ...
    if (dlt_user.dlt_is_file) {
        ret=dlt_offline_on_write(dlt_user.dlt_file_handle, msg.headerbuffer, msg.headersize, log->buffer, log->size);
    } 
    
    if(dlt_user.dlt_is_daemon) {
        dlt_user_log_reattach_to_daemon();
...
void generate_file_name(const char* prefix, char *log_file_name, int length)
{
	char filename[DLT_MOUNT_PATH_MAX] = {0};
	time_t t = time(NULL);
    char appid[DLT_ID_SIZE+1] = {0};
	struct tm *tm_info = localtime(&t);
	int index = 1;
    dlt_set_id(appid, dlt_user.appID);
	snprintf(filename, DLT_MOUNT_PATH_MAX,
		"%s/%s%04d%02d%02d-%02d%02d%02d.dlt",
        prefix,
        appid,
		...
		tm_info->tm_min,
		tm_info->tm_sec);
	while (index <= 10000 && access(filename, F_OK) == 0)
	{
		snprintf(filename, DLT_MOUNT_PATH_MAX,
			"%s/%s%04d%02d%02d-%02d%02d%02d(%04d).dlt",
			prefix,
            appid,
			1900 + tm_info->tm_year,
			1 + tm_info->tm_mon,
	...
	strncpy(log_file_name, filename, (length < DLT_MOUNT_PATH_MAX ? length : DLT_MOUNT_PATH_MAX));
    log_file_name[DLT_MOUNT_PATH_MAX-1] = 0;
}
...
static int dlt_offline_on_init(const char* path)
{
    ...
    if (path == 0)
    {
        dlt_log(LOG_ERR, "dlt_offline_on_init path must be given!\n");
        return DLT_RETURN_ERROR;
    }

    char appid[DLT_ID_SIZE+1] = {0};
    dlt_set_id(appid, dlt_user.appID);
    if (dlt_mkdir_recursive(path) != 0)
    {
      snprintf(str, DLT_USER_BUFFER_LENGTH, "Base dir %s cannot be created!\n", path);
      dlt_log(LOG_ERR, str);
      return -1;
    }
    count = dlt_offline_scan_file(&dlt_user.file_list, path, appid);
   if (dlt_user.file_list != 0 && count)
    {
        dlt_offline_sort_file(&dlt_user.file_list);
        curr = dlt_offline_pop_file(&dlt_user.file_list, dlt_user.file_count);
    }


	char log_file_name[DLT_MOUNT_PATH_MAX] = { 0 };
	
    generate_file_name(path, log_file_name, DLT_MOUNT_PATH_MAX);
    strcpy(dlt_user.path, path);
    if (count == 0)
    {
        dlt_user.dlt_file_handle = dlt_offline_open_file(log_file_name);
        dlt_user.file_list = dlt_user.dlt_file_handle;
    }
    else
    {
        if (curr == 0)
        {
            dlt_vlog(LOG_ERR, "curr is null\n");
            return -1;
        }
        curr->next = dlt_offline_open_file(log_file_name);
        dlt_user.dlt_file_handle = curr->next;
    }
...
static int dlt_offline_on_write(DltOfflineFileList *file, void* data1, int size1, void* data2, int size2)
{
    pthread_mutex_lock(&g_mutex_logOnWrite); 
    if (file == 0)
    {
        dlt_vlog(LOG_ERR, "dlt_offline_on_write arg file is null\n");
        pthread_mutex_unlock(&g_mutex_logOnWrite);
        return -1;
    }

    if (dlt_user.dlt_file_handle->size > dlt_user.file_size)
    {
        dlt_offline_close_file(dlt_user.dlt_file_handle);
        char log_file_name[DLT_MOUNT_PATH_MAX] = { 0 };
        generate_file_name(dlt_user.path, log_file_name, DLT_MOUNT_PATH_MAX);
        dlt_user.dlt_file_handle->next = dlt_offline_open_file(log_file_name);
        dlt_user.dlt_file_handle = dlt_user.dlt_file_handle->next;

        dlt_offline_pop_file(&dlt_user.file_list, dlt_user.file_count);
    }

    if (data1 != 0 && size1 >= 0)
    {
        dlt_offline_write_file(dlt_user.dlt_file_handle, data1, size1);
    }

    if (data2 != 0 && size2 >= 0)
    {
        dlt_offline_write_file(dlt_user.dlt_file_handle, data2, size2);
    }
    pthread_mutex_unlock(&g_mutex_logOnWrite);
    return 1;
}
...
{
    dlt_offline_close_file(dlt_user.dlt_file_handle);
    DltOfflineFileList *file = dlt_user.file_list;
    DltOfflineFileList *tmp = file;
    while (file)
    {
        file = file->next;
        free(tmp);
