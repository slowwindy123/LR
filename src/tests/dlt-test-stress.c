...
int main(int argc, char *argv[])
{
 ...
 /*  const char*Path="/tmp/APP3.dlt";
    uint32_t fileSize = 1073741824;

    int ret=0;
     if ((ret = dlt_init_file(Path)) < 0) {
            printf("logging: [" "] unable to init file log mode in DLT back-end!\n");
        }
        else{
            if((ret = dlt_set_filesize_max(fileSize)) < 0){
                printf("logging: dlt_set_filesize_max error!\n");
            }
            else{
                printf("Enabled File: ouput\n");
            }
        }*/
